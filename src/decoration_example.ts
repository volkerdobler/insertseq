...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand(
    'extension.insertNumberSequence',
    async () => {
      ...
      const updatePreview = (start: number, step: number = 1) => {
      ...
      await editor.edit((editBuilder) => {
        ...
        const sequenceText = Array.from({ length: lineCount }, (_, i) =>
          String(startValue + i * stepValue),
        ).join('\n');
        editBuilder.insert(cursorPos, sequenceText);
      });

      vscode.window.showInformationMessage(
        `Inserted sequence starting at ${startValue} (step ${stepValue})`,
      );
    },
  );
