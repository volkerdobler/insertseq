...
function tryIfToTernary(code) {
    const re = /^\s*if\s*\(([\s\S]*?)\)\s*\{\s*([\s\S]*?)\s*\}\s*else\s*\{\s*([\s\S]*?)\s*\}\s*$/i;
...
function runInVmAssignResult(code, timeout = 1000) {
    const vm = getVm();
    if (!vm)
        throw new Error('vm module not available in this environment');
...
function safeEvaluate(code, timeout = 1000) {
    // 1) try evaluating as an expression by assigning to `result` (works for '1+5')
    try {
        const exprAssign = runInVmAssignResult('result = (' + code + ')', timeout);
