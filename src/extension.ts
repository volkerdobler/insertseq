...
// own (war "months"): ;<start>[:<step>][#<repeat>][\*<frequency>][~<format>][@<stopexpr>][$][!]
	...
	const inputOptions: vscode.InputBoxOptions = {
		...
		validateInput: function(input) {
			currentSelection = insertNewSequence(editor, input, currentSelection, {origCursorPos: origCursorPositions, origTextSel: origTextSelections, segments: inputSegments, config: config});
			return "";
		}
	}
...
// i ::= counter, starting with 0 and increasing with each insertion
	...
	ruleTemplate.start_alpha                 = `^(?:(?<start>[a-z]+) (?= {{delimiter}} ))`;
	ruleTemplate.start_date                  = `^(?:(?:{{charStartDate}}) \\s* (?<start> (?<year>\\d{2}|\\d{4}) (?:(?:-(?<month>0?[1-9]|10|11|12))(?:-(?<day>0?[1-9]|[12][0-9]|30|31))?)? (?![\\d-]) (?:{{language}})? ) (?= {{delimiter}} ))`;
	...
	ruleTemplate.steps_date                  = `(?:(?<!{{charStartSteps}})(?:{{charStartSteps}}) \\s* (?<date_unit>[dwmy])? (?<steps> {{signedNum}}) (?= {{delimiter}} ))`;
	ruleTemplate.steps_other                 = `(?:(?<!{{charStartSteps}})(?:{{charStartSteps}}) \\s* (?<steps> {{signedInt}}) (?= {{delimiter}} ))`;
	ruleTemplate.format_decimal              = `(?: {{charStartFormat}} (?<padding> {{leadchars}} )? (?<align> [<>^=] )? (?<sign> [ +-] )? (?<length> \\d+ ) (?<precision>\\.\\d+)? (?<type>[bcdeEfFgGnoxX%]) (?= {{delimiter}} ) )`;
...
function insertNewSequence(
	...
	// const strList = currentSelection.map((_, idx) => strGenerator(idx));
...
function getSequenceFunction(input: string, p: TParameter): (i : number) => string {
...
function createDecimalSeq(input: string, parameter: TParameter, base: number): (i : number) => string {
// rules: RuleTemplate, selections: vscode.Selection[], config: vscode.WorkspaceConfiguration, base: number): (i : number) => string {

	const start = Number(input.match(parameter.segments["start_decimal"])?.groups?.start) || Number(parameter.config.getConfig('start')) || 1;
	const steps = Number(input.match(parameter.segments["steps_decimal"])?.groups?.steps) || Number(parameter.config.getConfig('step')) || 1;
	const repe = Number(input.match(parameter.segments["repetition"])?.groups?.repeat) || Number(parameter.config.getConfig('repetition')) || Number.MAX_SAFE_INTEGER;
	const freq = Number(input.match(parameter.segments["frequency"])?.groups?.freq) || Number(parameter.config.getConfig('frequency')) || 1;
	const startover = Number(input.match(parameter.segments["startover"])?.groups?.startover) || Number(parameter.config.getConfig('startover')) || Number.MAX_SAFE_INTEGER;
	...
	return (i) => {
		...
		specialValues.newValue = String(start + (steps * parseInt(((( i % startover) % (freq * repe)) / freq).toString(), base)));
		...
		try {
			let value = runExpression(replaceSpecialChars(stopexpr, specialValues));
...
function createStringSeq(input: string, parameter: TParameter): (i : number) => string {

	const start = input.match(parameter.segments["start"])?.groups?.start || "";
	if (start === "") return (i) => "\u{0}"

	const steps = Number(input.match(parameter.segments["steps_other"])?.groups?.steps) || Number(parameter.config.getConfig('step')) || 1;
	const repe = Number(input.match(parameter.segments["repetition"])?.groups?.repeat) || Number(parameter.config.getConfig('repetition')) || Number.MAX_SAFE_INTEGER;
	const freq = Number(input.match(parameter.segments["frequency"])?.groups?.freq) || Number(parameter.config.getConfig('frequency')) || 1;
	const startover = Number(input.match(parameter.segments["startover"])?.groups?.startover) || Number(parameter.config.getConfig('startover')) || Number.MAX_SAFE_INTEGER;
	const stopexpr = input.match(parameter.segments["stopexpression"])?.groups?.stopexpr || "";
	...
	const alphabet = parameter.config.getConfig('alphabet') || "abcdefghijklmnopqrstuvwxyz";

	// Prüfung, ob alphabet Duplikate enthält
	const uniqAlphabet = new Set(alphabet);
	if (uniqAlphabet.size !== alphabet.lentgh) {
		throw new Error("Alphabet includes invalid entries!");
	}

	const charToIndex = new Map<string, number>();
	alphabet.forEach((char: string, i: number) => charToIndex.set(char, i));
	...
	function stringToIndex(str: string): number {
		...
		for (const ch of chars) {
			...
			index = index * alphabet.length + charIndex;
		}

		// Alle Kombinationen kürzrer Länge berücksichtigen
		for (let len = 1; len < chars.length; len++) {
			index += Math.pow(alphabet.length, len)
		}
	...
	function indexToString(index: number): string {
		...
		while (true) {
			const combinations = Math.pow(alphabet.length, length)
		...
		for (let i = 0; i < length; i++) {
			chars.unshift(alphabet[index % alphabet.length]);
	...
	return (i) => {
		specialValues.origValue = parameter.origTextSel[i];
		specialValues.index = i.toString();
		specialValues.newValue = indexToString(Math.trunc(currentIndex + (steps * ((( i % startover) % (freq * repe)) / freq))));

		// if expression does not lead to a string, the current / new value will not be changed
		try {
			let value = runExpression(replaceSpecialChars(expr,specialValues));
			if (typeof value === "string") {
				specialValues.newValue = value;
			}
		} catch {
		}
...
function createDateSeq(input: string, parameter: TParameter): (i : number) => string {

	const start = input.match(parameter.segments["start_date"])?.groups?.start;
	if (!start) return (i) => "\u{0}";

	let yearStr = input.match(parameter.segments["start_date"])?.groups?.year || Temporal.Now.plainDateISO().year.toString();
	if (yearStr.length === 2) yearStr = parameter.config.getConfig("century") + yearStr;
	const year = Number(yearStr) || Temporal.Now.plainDateISO().year;
	const month = Number(input.match(parameter.segments["start_date"])?.groups?.month) || Temporal.Now.plainDateISO().month
	const day = Number(input.match(parameter.segments["start_date"])?.groups?.day) || Temporal.Now.plainDateISO().day
	const language = input.match(parameter.segments["start_date"])?.groups?.language || parameter.config.getConfig("language") || "de";

	const steps = Number(input.match(parameter.segments["steps_other"])?.groups?.steps) || Number(parameter.config.getConfig('step')) || 1;
	const unit = input.match(parameter.segments["start_date"])?.groups?.date_unit || parameter.config.getConfig("dateStepUnit") || "d";
	const repe = Number(input.match(parameter.segments["repetition"])?.groups?.repeat) || Number(parameter.config.getConfig('repetition')) || Number.MAX_SAFE_INTEGER;
	const freq = Number(input.match(parameter.segments["frequency"])?.groups?.freq) || Number(parameter.config.getConfig('frequency')) || 1;
	const startover = Number(input.match(parameter.segments["startover"])?.groups?.startover) || Number(parameter.config.getConfig('startover')) || Number.MAX_SAFE_INTEGER;
	const stopexpr = input.match(parameter.segments["stopexpression"])?.groups?.stopexpr || "";

	const instant = Temporal.PlainDate.from({ year: year, month: month, day: day })
	...
	return (i) => {
		...
		let idx = Math.trunc((steps * ((( i % startover) % (freq * repe)) / freq)));
		...
		if (idx >= 0) {
			...
			switch (unit) {
				case "w":
					...
					break;
				case "m":
					return instant.add({ months: idx }).toLocaleString(language)
					break;
				case "y":
			...
			switch (unit) {
				case "w":
					return instant.subtract({ weeks: Math.abs(idx) }).toLocaleString(language)
					break;
				case "m":
					return instant.subtract({ months: Math.abs(idx) }).toLocaleString(language)
					break;
				case "y":
					return instant.subtract({ years: Math.abs(idx) }).toLocaleString(language)
					break;
				default:
					return instant.subtract({ days: Math.abs(idx) }).toLocaleString(language)
			}
...
function createExpressionSeq(input: string, parameter: TParameter): (i : number) => string {
	return (i) => ""
}

function createOwnSeq(input: string, parameter: TParameter): (i : number) => string {
	return (i) => ""
}
